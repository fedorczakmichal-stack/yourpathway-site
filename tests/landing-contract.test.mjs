...
test("raw landing is English-first and client-facing", () => {
  ...
  assert.match(html, /Free open beta · v87/);
  assert.match(html, /Current v87 beta screens/);
  assert.match(html, /A living map for real life · beta v87/);
  assert.match(html, /var SITE_VERSION="87";/);
});
...
test("English social metadata uses a cache-busted 1200x630 asset contract", async () => {
  ...
  assert.match(html, /og:image" content="[^"]+\/img\/og-v87-en\.jpg"/);
  assert.match(html, /twitter:image" content="[^"]+\/img\/og-v87-en\.jpg"/);
  assert.match(html, /og:image:width" content="1200"/);
  assert.match(html, /og:image:height" content="630"/);
  assert.ok((await stat(new URL("../img/og-v87-en.jpg", import.meta.url))).size > 10_000);
  assert.ok((await stat(new URL("../img/og-v87-en.svg", import.meta.url))).size > 1_000);
...
test("the app is served from this origin, so the landing can read its telemetry", async () => {
  ...
  assert.equal(manifest.start_url, "./index.html?v=87");
  assert.equal(manifest.scope, "./");            // zakres /app/, nie koliduje z landingiem
  assert.match(worker, /const VERSION = "v87";/); // nowa nazwa cache'u kasuje poprzednią
  assert.match(appHtml, /<meta name="pathway-build" content="v87/);
